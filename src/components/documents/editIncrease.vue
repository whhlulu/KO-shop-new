<template>
  <div>
      ...
      <top-header :text="title" :editdelet='editdelet' :send="send"></top-header>
      <ul class="ordinary">
          <li>
              <p><span>公司名称：</span><input type="text" v-model="send.company_name"/></p>
          </li>
          <li>
              <p><span>税号：</span><input type="text" v-model="send.ein"/></p>
          </li>
          <li>
              <p><span>开户银行名称：</span><input type="text" v-model="send.opening_bank"/></p>
          </li>
          <li>
              <p><span>开户银行账号：</span><input type="text" v-model="send.bank_account"/></p>
          </li>
          <li>
	         <span>注册地区：</span>
	         <div class=" mint-field choose">
	            <div class="mint-cell-value" @click="callingArea">
	              <input placeholder="请选择地区" type="tel" class="mint-field-core" disabled v-model="addressAllOn">
          ...
          <li>
              <p><span>详情地址：</span><input type="text" v-model="send.register_address"/></p>
          </li>
          <li>
              <p><span>注册电话：</span><input type="tel" v-model="send.register_tel"  oninput="if(value.length > 11)value=value.slice(0, 11)"/></p>
          </li>
      </ul>
       <mt-popup v-model="popupVisible" position="bottom" class="mint-popup-3  pop">  
		...
		<div class="footer">
			<button class=" protect" @click="protact">确认修改</button>
		</div>
  </div>
</template>
<script >
...
export default {
    ...
    created(){
    	this.send=JSON.parse(sessionStorage.getItem("increase"));
    	this.addressAllOn=this.send.prov_name+"-"+this.send.city_name+"-"+this.send.dist_name;
    },
    mounted(){
        this.proAjax(0, 0)
    },
    methods: {
    	protact(){
            ...
            if(this.provinceId){
                data = {
                    id:this.send.id,
                    ein:this.send.ein,
                    company_name:this.send.company_name,
                    opening_bank:this.send.opening_bank,
                    bank_account:this.send.bank_account,
                    prov_id:this.provinceId,
                    city_id:this.cityId,
                    dist_id:this.areaId,
                    register_address:this.send.register_address,
                    register_tel:this.send.register_tel
                }
            }else{
                data = {
                    id:this.send.id,
                    ...
                    opening_bank:this.send.opening_bank,
                    bank_account:this.send.bank_account,
                    prov_id:this.send.prov_id,
                    city_id:this.send.city_id,
                    dist_id:this.send.dist_id,
                    register_address:this.send.register_address,
                    register_tel:this.send.register_tel
                }
            }
    		this.axios.post(this.$httpConfig.capitaSave,QS.stringify(
                data
                )).then((res) => {
        ...
        selectaddress() {
            ...
            if(this.province==""||this.province=="请选择") {
                ...
                this.addressAllOn = this.province + '-' + this.city + '-' + this.area;
