	...
	<div>
		...
		<div class="address-wrap" v-if="address">
			...
			<div class="con-center"> 
				<div class="personalInf clearfix">
					<span class="name fl"><em></em>{{address.realname}}</span>
					<span class="number fl">{{address.mobile}}</span>
				</div>
				<div class="address">{{address.prov_name + address.city_name + address.dist_name + address.realname}}</div>
				<div class="icon"></div>
			</div>
				...
				<li class="clearfix" v-for="(item,index) in order.goods" :key="index">
					...
					<div class="explain-wrap fr">
						...
						<p class="clearfix product" @click="enterShop(index,item)">
							<span class="fl price">￥{{item.goods_discount}}</span>
							<span class="fr number">x{{item.package_num}}</span>
						</p>
		...
		<div class="sundry-wrap">
			...
			<div class="money-wrap">
				...
				<p class="clearfix">
					...
					<span class="fr price">- ￥{{order.coupon_deductible||0}}</span>
				</p>
				<p class="clearfix">
		...
		<footer>
			<div class="btn-wrap">
				...
				<button class="fr" v-if="$route.params.status == -1" @click="goBuy">再次购买</button>
	...
	export default {
		...
		methods: {
			...
			cancelOrder() {
				MessageBox.confirm('确定取消订单吗?').then(action => {
					this.axios.post(this.$httpConfig.packageCancelOrder, qs.stringify({
						id: this.order.id
					})).then((res) => {
			...
			goBuy() {
					 ...
					 * 再次购买 （请求接口后直接跳转到购物车）
					 */
					this.axios.post(this.$httpConfig.packageBuyAgain,qs.stringify({
						id:this.order.id
					})).then((res)=>{
						if(res.data.status){
							Toast('添加成功!');
		...
		mounted() {
			...
			document.body.scrollTop = 0;
				this.axios.post(this.$httpConfig.packageOrderDetail, qs.stringify({
					...
					if(res.data.status == 10001) {
						...
						this.store_name = res.data.data.store.shop_name;
