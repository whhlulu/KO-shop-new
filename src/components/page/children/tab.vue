        ...
        <mt-tab-container v-model="selected">
            ...
            <mt-tab-container-item id="3">
                <div class="comment-wrap">
                    <ul>
                        <li @click="conActiveClick(index)" v-for="(item,index) in conName" :key="index"
                            :class="{conActive: index== comIndex}">
                            <span>{{item}}</span>
                    ...
                    <div v-else class="list-wrap" v-for="item in dataItem" :key="item.id">
                        ...
                        <div class="list-img-main clearfix" v-if="item.img.length>0">
                            <img :class="{'active':isChoose}" :src="URL + items.path" class="fl"
                                 v-for="items in item.img" :key="items.id">
                        </div>
                        <div class="list-replay" v-if="item.replay_comment">
                            系统回复： {{item.replay_comment}}
                            <span class="tran"></span>
                        </div>
                        <div class="list-purTimer">
                            {{item.goods_apace[0].speName}}&nbsp;&nbsp;&nbsp;{{item.goods_apace[0].item
                            }} &nbsp;&nbsp;&nbsp;{{formatTime(item.create_time) | formatDate}}
                        </div>
                    </div>
                </div>
            </mt-tab-container-item>
