<template>
    <div class="cashier">
        ...
        <div class="payment-wrap">
            <div class="status">
                ...
                <div class="pull-right fr">
                    <span>{{$route.params.number}}</span>元
                </div>
            </div>
        </div>
        <!--<div class="choice" >-->
        ...
        <dl class="other" v-if="$route.params.id != 3">
            <dt>支付方式</dt>
            <dd v-if="item.is_default!= 1" v-for="item in paymentType" :key="item.id" class="clearfix"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            wechatPay(){
                plus.webview.create(this.payData,'whh-weixin',{
                    additionalHttpHeaders: {
                        Referer: 'http://m.fzxbwl.com',
                    },
                })
                setTimeout(()=>{
                    this.$router.push({
                ...
                //var url = this.payData.split('&redirect_url')[0]
//                if (mui.os.android) {
//
//                } else {
...
//                }
            ...
            paymentMethod(id) {
                if (this.$route.params.id != 3) {
                    this.load = true
                    var that = this;
                    this.axios.post(this.$httpConfig.initiatePayment, qs.stringify({
                        pay_type_id: id,
                        platform: 1
                    })).then(function (res) {
                        that.load = false
