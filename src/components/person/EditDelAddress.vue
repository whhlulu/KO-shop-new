<template>
    <div class="edit-wrap">
        ...
        <footer-btn :text="footBnt" :id="$route.params.status" :to="'/address'" :data="data" :province="provinceId" :street ='streetId' :city="cityId" :area="areaId"></footer-btn>
    </div>
</template>
<script>
            ...
            getSingleAddress(){
                this.axios({
                        ...
                        if(res.data.status == 1){
                            ...
                            this.addressAll = this.data.prov_name +'-'+ this.data.city_name +'-'+this.data.dist_name;
                        }
					}).catch((err) => {
						console.log(err);
					});
