			...
			<div class="item-wrap">
				<dl class="item-main clearfix" v-for="item in data_con" :key="item.id">
					...
					<dd class="fl clearfix text-center">
						<span class="fl" v-for="items in item.typeTwo" :key="items.id" @click="tolink(items)">{{items.name}}</span>
					</dd>
				</dl>
			</div>
		</div>
		<Shopsn></Shopsn>
		<div class="load-wrap" v-show="load" @touchmove.prevent>
...
<script>
	import qs from 'qs';
	import customerHeader from '@/components/page/children/header.vue';
	...
	export default {
		...
		mounted() {
			this.axios.post(this.$httpConfig.home).then((res) => {
				this.data = res.data.data.announcement;
				var N = 0;
				this.change(N);
				setInterval(() => {
					N++;
			...
			// 文章分类
			this.axios.post(this.$httpConfig.helpClass).then((res) => {
				this.load = false;
		...
		methods: {
			...
			phoneMsg() {
				MessageBox({
					...
					if(action == "confirm") window.location.href = "tel:17864160302"
				})
			},
		},
		components: {
