<template>
	<div>
		...
		<ul class="per-center">
			<li>
				<span>微&nbsp;&nbsp;信:</span><input type="text" placeholder="请输入微信号码" v-model="wechatNum">
			</li>
			<li>
				<span> 支付宝:</span><input type="text" placeholder="请输入支付宝号码" v-model="alipayNum">
			</li>
			<li>
				<span> 开户行:</span><input type="text" placeholder="请输入银行开户行" v-model="bank_name">
			</li>
			<li>
				<span> 银行卡:</span><input type="text" placeholder="请输入银行卡号码" v-model="bankNum">
			</li>
		</ul>
...
export default {
	...
	methods :{
		nextinfor: function () {
			...
			if (!/(^[1-9]([0-9]*)$|^[0-9]$)/.test(this.bankNum) || this.bankNum.length<16){
				Toast({
					message: '请输入正确的银行卡号',
					duration: 1000
				});
...
#address-area {
  ...
  bottom: 0;
  background: #fff;
  box-shadow: 0 -4px 7px rgba(255, 255, 255, 0.6);
}

.address-area-tit {
  border-bottom: 1px solid #ccc;
  line-height: 40px;
  box-shadow: 0px 1px 2px rgba(23, 43, 135, 0.2);
}
